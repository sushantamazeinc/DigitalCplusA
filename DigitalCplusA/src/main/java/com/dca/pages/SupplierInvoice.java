...
public class SupplierInvoice extends BaseClass {
	...
	@FindBy(xpath="//input[@id='txt_purchaseitemDate']")
	WebElement date;
	...
	public void supplierinvoice(String selproject1, String date1, String invoiceno1,
			String warrentydate1, String mfname1, String partno1, String actualquanity1, String mrp1, String discount1,
			String comments1, String selpaymenttype1, String comment1, String returnquanity1,
			String remarks1, String address11, String floor1, String location1, String currentquanity1,
			String minimumquantity, String expirydate12, String assignedto1, String description1, String selcat,
			String selty, String staff1) throws InterruptedException, AWTException {
		ac.selByVisibleText(selproject, selproject1);
		//clicking.click();
	
		
		/*
		 * addsupplierinvoice.click(); Thread.sleep(3000); Select select22=new
		 * Select(selsupplier); int selectoptions= select22.getOptions().size();
		 * select22.selectByIndex(selectoptions - 1); //selsupplier.click();
		 * Thread.sleep(3000); Select select23=new Select(searchpono); int
		 * selectop=select23.getOptions().size(); select23.selectByIndex(selectop - 1);
		 * //ac.selByVisibleText(selsupplier2, suppname);
		 * //ac.selByVisibleText(searchpono, ponumbb); date.sendKeys(date1);
		 * invoiceno.sendKeys(invoiceno1); ac.selByIndex(itemname, 1);
		 * warrentydate.sendKeys(warrentydate1); mfname.sendKeys(mfname1);
		 * partno.sendKeys(partno1); actualquantity.sendKeys(actualquanity1);
		 * mrp.sendKeys(mrp1); discount.sendKeys(discount1);
		 * comments.sendKeys(comments1); save1.click(); saveok1.click();
		 * Thread.sleep(3000); search1.sendKeys(invoiceno1);
		 * //ac.selByVisibleText(selsuppliers3, text); download12.click();
		 * Thread.sleep(3000); supplieroutstanding.click(); Thread.sleep(3000); Select
		 * select27=new Select(selsuppliers4); int sel27=select27.getOptions().size();
		 * select27.selectByIndex(sel27 - 1);
		 * 
		 * //ac.selByVisibleText(selsuppliers4, text); export1.click(); pay.click();
		 * Thread.sleep(3000); Robot robot=new Robot(); for(int i=0;i<1;i++) {
		 * robot.keyPress(KeyEvent.VK_PAGE_DOWN); Thread.sleep(3000); }
		 * ac.selByVisibleText(selpaymenttype, selpaymenttype1); String
		 * payable=getamount.getAttribute("value"); payableamount.clear();
		 * Thread.sleep(3000); payableamount.sendKeys(payable);
		 * comment.sendKeys(comment1); payment.click(); paymentok.click();
		 * //paymentreports.click(); Thread.sleep(3000); ac.selByIndex(selsupplier12,
		 * 1); Thread.sleep(3000); export12.click(); //ac.selByIndex(selsupplier12, 1);
		 * //ac.selByVisibleText(selsupplier12, text); download1.click();
		 * Thread.sleep(3000);
		 */
		/*
		 * supplierreturn.click(); Robot robot=new Robot(); for(int i=0;i<1;i++) {
		 * robot.keyPress(KeyEvent.VK_PAGE_UP); Thread.sleep(3000); } addnew.click();
		 * Thread.sleep(3000); //ac.selByIndex(selsupplier123, 11); Thread.sleep(3000);
		 * Select select28=new Select(selsupplier123); int
		 * sel289=select28.getOptions().size(); select28.selectByIndex(sel289-1);
		 * 
		 * 
		 * 
		 * Select select30=new Select(selinvoicenumber); int
		 * sel30=select30.getOptions().size(); select30.selectByIndex(sel30-1);
		 * //ac.selByVisibleText(selsupplier123, text);
		 * //ac.selByVisibleText(selinvoicenumber, invoiceno1);
		 * //ac.selByIndex(selinvoicenumber, 1); returnquanity.sendKeys(returnquanity1);
		 * remarks.sendKeys(remarks1); sel.click(); returnpro.click();
		 * yesreturn.click(); returnok.click(); Thread.sleep(3000);
		 */
	assertclick.click();
